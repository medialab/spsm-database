...
def get_results_for_one_query(url_id, query, rcfile, output_dir, errors_writer, results_writer, not_academic):
    ...
    minet_results_dir = os.path.join(output_dir, 'minet_outfiles')
    os.makedirs(minet_results_dir, exist_ok=True)
    query_outfile = os.path.join(minet_results_dir, unique_file_name(url_id=url_id))
...
def concatenate_filepath(url_id):
    ...
    time = time.split('.')[0]
    return Path(f'{url_id}_{time}.csv')


def unique_file_name(url_id):
    filepath = concatenate_filepath(url_id)
    if filepath.is_file():
        sleep(1)
        filepath = concatenate_filepath(url_id)
